do $$ declare cid uuid := 'cccc0001-0000-4000-8000-000000000008'; begin
 ...
 begin insert into claims(initiative_id,type,status,subject,attribute,value,domain,origin) values('11111111-1111-4111-8111-111111111111','RISK','ACTIVE','Human','Active','No trust','RISK','HUMAN_ENTRY'); raise exception 'human active accepted'; exception when check_violation then null; end;
...
-- Explicit LEGACY inserts remain possible for the committed seed and do not
-- manufacture verification history for grandfathered active claims.
insert into claims(id,initiative_id,type,status,subject,attribute,value,domain,origin)
values('dddd0000-0000-4000-8000-000000000020','11111111-1111-4111-8111-111111111111','RISK','ACTIVE','Legacy','Grandfathered','Value','RISK','LEGACY');
do $$ begin
 if exists(select 1 from claims where id='dddd0000-0000-4000-8000-000000000020' and verified_at is not null) then
   raise exception 'legacy insert fabricated verification';
 end if;
end $$;
delete from claims where id='dddd0000-0000-4000-8000-000000000020';
