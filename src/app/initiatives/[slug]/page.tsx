  ...
  return <div className={styles.page}>
    ...
    <section className={styles.section}>
      ...
      {open.length ? <ul className={styles.list}>{open.map((finding) => <li key={finding.fingerprint}>
        ...
        <div className={styles.attentionActions}>
          ...
          <details><summary>Why raised</summary><p>{MISMATCH_WHY_RAISED}</p></details>
    ...
    <section className={styles.section}>
      ...
      {activity.length ? <ul className={styles.list}>{activity.map((entry) => <li key={entry.id}>
        <span className={styles.change}><strong data-activity-summary data-activity-legacy={!isStructuredActivity(entry) || undefined}>{activitySummary(entry)}</strong><time dateTime={entry.occurredAt}>{new Date(entry.occurredAt).toLocaleDateString("en-GB")}</time></span>
